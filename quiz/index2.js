var quiz = {
  ...
  init: () => {
    ...
    // (B4) ADD CATEGORY SELECTION INTERFACE
  ...
  // (B6) CREATE CATEGORY SELECTION INTERFACE
  createCategorySelectionInterface: () => {
    ...
    // (B7) ADD EVENT LISTENER TO START QUIZ BUTTON
  ...
  draw: () => {
    // (C1) QUESTION
    const categoryQuestions = quiz.categories[quiz.selectedCategory];
    ...
    if (quiz.now < categoryQuestions.length) {
      ...
      // (C2) OPTIONS
      quiz.hAns.innerHTML = "";
      for (let i in categoryQuestions[quiz.now].o) {
      ...
      // (C3) QUIZ ENDS
      quiz.hQn.innerHTML = `You have answered ${quiz.score} of ${categoryQuestions.length} correctly.`;
      quiz.hAns.innerHTML = "";
    }
  },
  ...
  select: (option) => {
    ...
    const currentQuestion = categoryQuestions[quiz.now];
    let correct = option.dataset.idx == currentQuestion.a;
    if (correct) {
      quiz.score++;
